...
class ZjmfFinanceApiController extends CommonController
{
	 ...
	 * @list  create_time:
	 * @list  description:
	 * @list  description_parts:结构化描述片段
	 * @list  ip:
	 * @list  username:
	 */
	public function apiLog()
	{
		...
		$logs = \think\Db::name("api_resource_log")->alias("a")->field("a.id,a.create_time,a.description,a.ip,b.username,a.port")->leftJoin("clients b", "a.uid = b.id")->where($where)->withAttr("ip", function ($value, $data) {
			if (empty($data["port"])) {
				return $value;
			} else {
