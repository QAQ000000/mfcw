...
创建工单的查询只硬性要求：工单关联产品和部门命中同一条规则，且主机存在大于 0 的
`dcimid`。当前实现未按产品 `api_type` 过滤，部门上游映射也是左连接；随后直接使用产品的
`zjmf_api_id` 登录，并把缺失的 `upstream_dptid` 转成 `0` 上送。因此“产品是可用的上游
API 产品”和“部门已配置大于 0 的上游 ID”是必须在发布检查中保证的配置前提，不是代码
已经兜底的条件。回复还要求原工单已经保存非空 `upstream_tid`。
...
对应 API 部门映射，不会让当前查询自动跳过；代码会以 `dptid=0` 请求上游，可能产生失败
请求或被异常路由。发布前必须校验映射值大于 0。
